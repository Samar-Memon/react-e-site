import React, { useEffect, useState } from 'react'
import '../../../App.css'
import{
  getFirestore,
  collection,
  addDoc,
  db,
  getDocs
} from '../../../FireBase/firebase'
import Card from '../../Card/Card';
import { useNavigate } from 'react-router-dom';

const Shirt = () => {

  const navigate = useNavigate()

  const [getShirtProduct, setGetShirtProduct] = useState([]);
  const [noData, setNoData] = useState(false);


  const getShirtDataFunc = async() => {
    const querySnapshot = await getDocs(collection(db, "all_product"));
    querySnapshot.forEach((doc) => {
      let filteredProducts = [];

    querySnapshot.forEach((doc) => {
      if (doc.data().productCategory === 'shirt') {
        filteredProducts.push(doc.data());
      } else {
        setNoData(true)
      }
    });

    // Update the state once with the complete list
    setGetShirtProduct(filteredProducts);
    });
  }

  useEffect(() => {
    getShirtDataFunc()
  }, [])

  return (
    <>
  <button className='border-2 border-[#00a6bb] text-white bg-[#00a6bb] py-2 px-6 rounded-md hover:bg-transparent hover:text-[#00a6bb] mt-3 ml-3' onClick={() => navigate('/')}><i className="fa-solid fa-arrow-left"></i> Back</button>
    <h1 className='text-3xl mt-10 mb-5 font-semibold text-[#00a6bb] text-center border-b-2 border-[#00a6bb] pb-5'>Shirts Items</h1>
    <div className='flex flex-wrap justify-evenly px-2 gap-y-5'>
    {
      getShirtProduct.length > 0 ? getShirtProduct.map((data, index) => {
        return(
          <Card key={index} image={data.productImageURL} title={data.productName} price={data.productPrice} desc={data.productDescription}/>
        )
      }) : noData ? <h1 className='text-2xl text-[#00a6bb] font-semibold'>No Data from Shirts Items</h1> : <div className="loader"></div>
    }
    </div>
    </>
  )
}

export default Shirt